...
def convert_block(data):
    domain_list = [line.strip() for line in data.splitlines() if line and not line.startswith("#") and not line.startswith("0.0.0.0")]
...
def convert_adway(data):
    ...
    domain_list = [domain for domain in domain_list if not domain.startswith("0.0.0.0")]
    return {"version": 1, "rules": [{"domain": domain_list}]}
...
def convert_MVPS(data):
    ...
    for line in data.splitlines():
        ...
        if line.startswith("# [Start of entries generated by MVPS HOSTS]"):
            ...
            continue
        if start_conversion and line and not line.startswith("#") and "localhost" not in line and not line.startswith("0.0.0.0"):
            parts = line.split()
    ...
    for line in data.splitlines():
        ...
        if line and not line.startswith("!") and "github.com" not in line:
            ...
            if len(parts) >= 2:
                ...
                if not domain.startswith("0.0.0.0"):
